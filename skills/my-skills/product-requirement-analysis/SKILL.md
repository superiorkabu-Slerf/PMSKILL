...
description: 对模糊产品想法、需求方向或新功能机会做分阶段需求验证。覆盖收窄需求、动态追问、分波次市场调研、竞品分析、用户声音挖掘、综合结论、验证校验和压力测试。适用于判断一个需求是否真实存在、是否值得投入、切入点在哪里、下一步该如何验证。
---
...
所有 research waves 完成后，先读完 `01-discovery/raw/` 下的全部 raw 文件，再开始综合。
